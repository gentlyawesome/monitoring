...
class Account extends MY_Controller {
	...
	public function change_password($id= false)
	{
		...
		if($id !== $this->userid){ show_404(); }

		$this->view_data['system_message'] = $this->session->flashdata('system_message');

		if($this->input->post('change_secreto') && $this->input->post('change_secreto') === "SAVE"){

			
			$new_secreto = $this->view_data['new_secreto'] = trim($this->input->post('new_secreto'));
			$con_secreto = $this->view_data['con_secreto'] = trim($this->input->post('con_secreto'));
			$old_secreto = $this->view_data['old_secreto'] = trim($this->input->post('old_secreto'));
			// vp($new_secreto);
			// vp($con_secreto);
			// vp($old_secreto);
			// die('x');
			if($new_secreto == "" || $con_secreto == "" || $old_secreto == ""){
				$this->_msg('e','Fill all the fields first', current_url());
			}

			if($new_secreto != $con_secreto){
				$this->_msg('e','New Password and Confirm Password do not match', current_url());
			}


			if(!$this->login->_validate_password($this->userlogin,$old_secreto)){
				$this->_msg('e','Current Password wrong', current_url());		
			}

			$this->login->_generate_password($new_secreto);
			unset($users);
			$users['crypted_password'] = $this->login->get_password();
			$users['salt'] = $this->login->get_salt();
			$rs = $this->M_users->update($id, $users);

			if($rs){
				$this->_msg('s','Password was changed.', current_url());		
			}else{
				$this->_msg('e','Process Failed. Please try again', current_url());		
			}
